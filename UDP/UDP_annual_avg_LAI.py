"""
UDP to generate an annual average LAI product from the Copernicus LAI datasets

Export algorithm to UDP json, e.g.

    python UDP/UDP_annual_avg_LAI.py > UDP/json/udp_annual_avg_lai.json
...
datacube2 = connection.load_collection(
    "CGLS_LAI_V2_GLOBAL", temporal_extent=[start, end], bands=["LAI"]
)

cube = if_(gte(param_year, 2015), datacube1, datacube2)

# masking to valid data and rescaling
cube = cube.apply(lambda x: if_(and_(x >= 0, x <= 210), x / 30.0))
...
description = """
Given a year and area of interest, returns a mean composite of [LAI](https://land.copernicus.eu/global/products/lai).
"""

spec = build_process_dict(
    process_id="udp_annual_avg_lai",
