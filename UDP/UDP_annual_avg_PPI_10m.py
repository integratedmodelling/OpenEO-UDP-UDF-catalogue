"""
UDP to generate an annual average PPI in high resolution (10m) from the Copernicus
High Resolution Vegetation Phenology and Productivity (HR-VPP) product suite
...
param_year = Parameter.integer(
    name="year",
    default=2021,
    description="The year for which to generate an annual mean composite. from 2017 onwards.",
)
...
cube = cube.apply(lambda x: if_(and_(x >= 0, x <= 30000), x / 10000.0))
...
description = """
Given a year and area of interest, returns an annual mean composite of PPI (https://collections.eurodatacube.com/seasonal-trajectories/).
"""

spec = build_process_dict(
    process_id="udp_annual_avg_ppi_10m",
    summary="Annual mean composite of Copernicus PPI. Returns a single band RasterCube.",
